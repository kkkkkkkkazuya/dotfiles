...
# wezterm設定ファイルのリンク作成
echo "Deploying wezterm configuration files..."
wezterm_config_dir="$HOME/.config/wezterm"
mkdir -p "$wezterm_config_dir"

for wezterm_file in wezterm.lua keybinds.lua; do
  if [[ -f "$dotfiles_dir/wezterm/$wezterm_file" ]]; then
    echo "Linking wezterm/$wezterm_file to $wezterm_config_dir/$wezterm_file"
    ln -sf "$dotfiles_dir/wezterm/$wezterm_file" "$wezterm_config_dir/$wezterm_file"
  fi
done

# neovim設定ファイルのリンク作成
echo "Deploying neovim configuration files..."
mkdir -p "$HOME/.config"
nvim_config_dir="$HOME/.config/nvim"
if [[ -L "$nvim_config_dir" || ! -e "$nvim_config_dir" ]]; then
  ln -sf "$dotfiles_dir/nvim" "$nvim_config_dir"
  echo "Linking nvim to $nvim_config_dir"
fi
